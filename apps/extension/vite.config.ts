...
// backed by chrome.storage (BL-014). The update channel (BL-015) is a separate
// publish build — see vite.channel.config.ts, chained in the build script.
export default defineConfig({
  ...
  test: {
    ...
    coverage: {
      ...
      include: ['src/**/*.ts'],
      // The entry shells are wiring, not logic.
      exclude: ['src/**/*.test.ts', 'src/content.ts', 'src/channel-entry.ts'],
    },
  },
});
